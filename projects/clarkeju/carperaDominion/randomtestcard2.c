...
int main() {
    ...
    while (count < 20) {
		...
		// call card function
		// village not refactored, so call cardEffect directly
		cardEffect(village, 0, 0, 0, &testG, handPos, &bonus);

		// test number of new cards
		// note: expect 0 new cards
		...
		printf("New deckCount: %d, Expected: %d ", testG.deckCount[0], G.deckCount[0] - 1);
		if (compareInt(testG.deckCount[0], G.deckCount[0] - 1) == 0) {
			failCount += 1;
		}

		// test number of actions
		printf("Actions: %d, Expected: %d ", testG.numActions, G.numActions + 2);
		if (compareInt(testG.numActions, G.numActions + 2) == 0) {
			failCount += 1;
		}
