...
public class UrlValidatorTest extends TestCase {
    ...
    public void testValidScheme() {
        ...
        {
            wholeUrl = validSchemes[i] + "www.google.com";
    ...
    public void testInvalidScheme() {
        ...
        {
            wholeUrl = invalidSchemes[i] + "www.google.com";
            assertFalse(urlVal.isValid(wholeUrl));
        }
    }
    ...
    public void testValidAuthority() {
        UrlValidator urlVal = new UrlValidator(null, null, UrlValidator.ALLOW_ALL_SCHEMES);
        String wholeUrl;
        String[] validAuthority = {"www.google.com", "www.apache.org", "0.0.0.0", "255.255.255.255", "155.com", "go.cc", "go.au"};

        for(int i = 0; i < validAuthority.length; i++)
        {
            wholeUrl = "http://" + validAuthority[i];
            assertTrue(urlVal.isValid(wholeUrl));
        }
    }
    ...
    public void testInvalidAuthority() {
        UrlValidator urlVal = new UrlValidator(null, null, UrlValidator.ALLOW_ALL_SCHEMES);
        String wholeUrl;
        String[] invalidAuthority = {"256.256.256.256", "4.5.6.7", ".4.5.6.7", "hi.a", "hello.a2a", "hi.2bb", "ccc.", ".ddd", ""};

        for(int i = 0; i < invalidAuthority.length; i++)
        {
            wholeUrl = "http://" + invalidAuthority[i];
            assertFalse(urlVal.isValid(wholeUrl));
        }
    }
    ...
    public void testValidPort() {
        ...
        {
            wholeUrl = "http://www.google.com" + validPort[i];
            assertTrue(urlVal.isValid(wholeUrl));
        }
    }
    ...
    public void testInvalidPort() {
        UrlValidator urlVal = new UrlValidator(null, null, UrlValidator.ALLOW_ALL_SCHEMES);
        String wholeUrl;
        String[] invalidPort = {":65536", ":-5", ":63537", ":777777777777", ":44d"};

        for(int i = 0; i < invalidPort.length; i++)
        {
            wholeUrl = "http://www.google.com" + invalidPort[i];
            assertFalse(urlVal.isValid(wholeUrl));
        }
    }
    ...
    public void testValidPath() {
        UrlValidator urlVal = new UrlValidator(null, null, UrlValidator.ALLOW_ALL_SCHEMES);
        String wholeUrl;
        String[] validPath = {"/this1", "/this123", "/this1/", "/this1/that", "/$45", ""};
    ...
    public void testInvalidPath() {
        UrlValidator urlVal = new UrlValidator(null, null, UrlValidator.ALLOW_ALL_SCHEMES);
        String wholeUrl;
        String[] invalidPath = {"/..", "/../", "/..//that", "/this//that"};

        for(int i = 0; i < invalidPath.length; i++)
        {
            wholeUrl = "http://www.google.com" + invalidPath[i];
            assertFalse(urlVal.isValid(wholeUrl));
        }
    }
    ...
    public void testValidOptions() {
        ...
        String wholeUrl;
        String[] validOptions = {"/this//that", "/$45/that", "", "/this1"};

        for(int i = 0; i < validOptions.length; i++)
        {
            wholeUrl = "http://www.google.com" + validOptions[i];
            assertTrue(urlVal.isValid(wholeUrl));
        }
    }
    ...
    public void testInvalidOptions() {
        ...
        UrlValidator urlVal = new UrlValidator(null, null, options);
        String wholeUrl;
        String[] invalidOptions = {"/#", "/..", "/#/that"};

        for(int i = 0; i < invalidOptions.length; i++)
        {
            wholeUrl = "http://www.google.com" + invalidOptions[i];
            assertFalse(urlVal.isValid(wholeUrl));
        }
    }
    ...
    public void testValidParts() {
        UrlValidator urlVal = new UrlValidator(null, null, UrlValidator.ALLOW_ALL_SCHEMES);
        String[] wholeUrls = new String[8];
        ...
        wholeUrls[0] = validScheme + validAuthority;
        wholeUrls[1] = validScheme + validAuthority + validPort;
        wholeUrls[2] = validScheme + validAuthority + validPort + validPath;
        wholeUrls[3] = validScheme + validAuthority + validPort + validPath + validQuery;
        wholeUrls[4] = validScheme + validAuthority + validPath;
        wholeUrls[5] = validScheme + validAuthority + validQuery;
        wholeUrls[6] = validScheme + validAuthority + validPath + validQuery;
        wholeUrls[7] = validScheme + validAuthority + validPort + validQuery;

        for(int i = 0; i < wholeUrls.length; i++)
        {
            assertTrue(urlVal.isValid(wholeUrls[i]));
        }
    }
    ...
    public void testInvalidParts() {
        ...
        String invalidAuthority = "go.a1a";
        String invalidPort = ":-1";
        String invalidPath = "/..";

        wholeUrls[0] = invalidScheme + invalidAuthority;
        wholeUrls[1] = invalidScheme + invalidAuthority + invalidPort;
        wholeUrls[2] = invalidScheme + invalidAuthority + invalidPort + invalidPath;
        wholeUrls[4] = invalidScheme + invalidAuthority + invalidPath;

        for(int i = 0; i < wholeUrls.length; i++)
        {
            assertFalse(urlVal.isValid(wholeUrls[i]));
        }
    }                     
    ...
    public void testValidRandom() {
        long options = UrlValidator.ALLOW_2_SLASHES + UrlValidator.ALLOW_ALL_SCHEMES + UrlValidator.NO_FRAGMENTS;
        UrlValidator urlVal = new UrlValidator(null, null, UrlValidator.ALLOW_ALL_SCHEMES);
        UrlValidator urlValOptions = new UrlValidator(null, null, options);
        ...
        for(int i = 0; i < 50; i++)
        {
            ...
            int scheme = rand.nextInt(validSchemes.length);
            wholeUrl += validSchemes[scheme];
            int authority = rand.nextInt(validAuthority.length);
            wholeUrl += validAuthority[authority];
            int port = rand.nextInt(validPort.length);
            wholeUrl += validPort[port];
            int path = rand.nextInt(validPath.length);
            wholeUrl += validPath[path];
            int query = rand.nextInt(validQuery.length);
            wholeUrl += validQuery[query];
        ...
        for(int i = 0; i < 15; i++)
        {
            ...
            int port = rand.nextInt(validPort.length);
            wholeUrl += validPort[port];
            int option = rand.nextInt(validOptions.length);
            wholeUrl += validOptions[option];
            int query = rand.nextInt(validQuery.length);
            wholeUrl += validQuery[query];

            assertTrue(urlValOptions.isValid(wholeUrl));
        }
    ...
    public void testInvalidRandom() {
        ...
        for(int i = 0; i < 50; i++)
        {
            ...
            int scheme = rand.nextInt(invalidSchemes.length);
            wholeUrl += invalidSchemes[scheme];
            int authority = rand.nextInt(invalidAuthority.length);
            wholeUrl += invalidAuthority[authority];
            int port = rand.nextInt(invalidPort.length);
            wholeUrl += invalidPort[port];
            int path = rand.nextInt(invalidPath.length);
            wholeUrl += invalidPath[path];
        ...
        for(int i = 0; i < 15; i++)
        {
            ...
            int port = rand.nextInt(invalidPort.length);
            wholeUrl += invalidPort[port];
            int option = rand.nextInt(invalidOptions.length);
            wholeUrl += invalidOptions[option];

            assertFalse(urlValOptions.isValid(wholeUrl));
        }
